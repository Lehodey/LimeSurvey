...
function OSGeoInitialize(question,latLng){
		...
		function UI_update(lat,lng){
			...
			// $.getJSON("http://api.geonames.org/findNearbyJSON?lat=" + lat + "&lng=" + lng + "&username=limesurvey&maxRows=1",function(result){
				// alert(result.geonames[0].geonameId);
			// });
			//geonameId = "";
			//alert($("#boycott_"+name).val());
			if ($("#boycott_"+name).val().indexOf("4") ==-1) { // state = True then store geonameid
			...
			if (isvalidCoord(lat) && isvalidCoord(lng)) {
				...
				if (geonameId) {
				$("#answer"+name).val(Math.round(lat*100000)/100000 + ";" + Math.round(lng*100000)/100000 + ";" + geonameId);
				} else {
...
/* Typeahead search functionality */
		  ...
		  $("#searchbox").typeahead({
			...
			templates: {
			  ...
			  header: "<h3 class='typeahead-header'>&nbsp;GeoNames</h3>"
			}
